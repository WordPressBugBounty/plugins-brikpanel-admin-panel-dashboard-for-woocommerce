...
= 3.3.6 (2026-09-15) =
* Fix: **Product costs are no longer counted twice.** When a product's cost was stored twice in the database, reports added both copies, so 5 items at £8.99 showed as £89.90 on the dashboard. Every report now reads the same single cost the product page shows: dashboard, cost coverage, Store Summary, the ads dashboard and the products list. WooCommerce's Duplicate and CSV imports with a cost column no longer write the cost twice, for simple products and every variation.
* New: **BrikControl check "Product Cost Records".** It lists products carrying extra copies of their cost and removes them with "Remove extra copies". The cost you see does not change, and "Undo" puts the copies back.
* Fix: **Several other figures that were counted wrong.** An order with two coupons was counted twice in coupon revenue, a customer who bought both as a guest and as a member was counted twice as returning, marketplace category revenue double counted some SKUs and missed variation SKUs, and on stores without HPOS country sales and city counts always showed 0. On those stores the orders overview total is now as precise as on HPOS stores.
* Tweak: **Store Summary "Average product price" counts a variable product once,** at its starting price, instead of once for every different variation price.
* New: **Compact order list.** Each order is now one short, readable row. Click the arrow to see the billing and shipping address, phone, payment and shipping method and the items (with variation details such as colour and size) without leaving the list. On phones each order is a card that opens on tap. On by default; switch it off under WooCommerce > Settings > BrikPanel > Orders List.
* New: **A cleaner orders list.** Screen Options and Add order sit side by side in the header, the overview is one full width card, the search button opens smoothly (Esc closes it), and status, payment and shipping are shown as small pills and icons. On narrow screens columns hide themselves instead of pushing the table sideways, and the bulk bar turns into a "Bulk actions" menu.
* New: **The order page is organised in tabs.** Items, Customer, Notes and More sit in one card on the left, with a summary (total, payment status, customer and WhatsApp), order actions and shipping cost fixed on the right. WooCommerce's own boxes are moved, not rebuilt, and boxes from other plugins go under More. Saving brings you back to the same tab. Part of the "Modern order edit" setting.
* Fix: **The Add order page now gets the modern design.** It used to load the orders list code instead. The header says "New order" with a "Create" button and a status picker, with HPOS on or off.
* Fix: **The order page works on tablets and phones.** One column with the summary on top, items shown as cards, edit and refund forms that fit the screen, and a tab bar that no longer hangs in the middle of the screen. Totals and addresses line up, right-to-left languages included.
* Fix: **Opening an order preview no longer jumps the page to the top.**
* Fix: **Screen Options on the order page is visible again,** and switching tabs no longer makes WordPress save boxes as hidden.
* Fix: **A single error on an empty store no longer stops the whole orders list layout,** and the WhatsApp column no longer shifts when the table scrolls sideways.
* New: **Tags, Brands, Categories, Attributes and attribute terms use the same clean design as the orders list.** The pages use the full width and "Add new" opens a side panel, so you can add several terms in a row without the form taking up half the screen. The table has search with a clear button, a bulk bar and simpler pagination; Attributes gains Screen Options, and edit pages are a centred card with a back button.
* Fix: **The default category row (Uncategorized) no longer loses its name and links on WordPress 7.** A WooCommerce script mistook the first cell for a checkbox and emptied it.
* Fix: **Smaller fixes on the term screens.** Errors such as "Tag not added." are red, focus outlines only show when you use the keyboard, the note under the table reads as a footnote, and WoodMart's form styles no longer leak in.
* New: **A "Coming soon" badge in the top bar.** While your site, or only your store pages, is set to Coming soon under WooCommerce's Site visibility, an orange badge beside the logo tells you so and opens that setting. It follows WooCommerce's own rules for who sees it. A long store name now ends with "…" instead of being cut mid letter.
* Dev: **New filter `brikpanel_cartab_popup_coupon_individual_use`** decides whether the abandoned cart popup coupon can be combined with other coupons. Off by default, so nothing changes.
...
= 3.3.2 (2026-09-13) =
* Fix: **The Settings button on Abandoned Carts is no longer a dead end for shop managers.** With "Restrict settings to administrators" on (the default), a shop manager who clicked it was silently dropped on WooCommerce → General, because the BrikPanel settings tab is closed to them. The button is now only shown to people who can actually open that tab, which also keeps it out of sight when you run the store on a client's behalf.
* Tweak: **The WhatsApp icon is green on Abandoned Carts, and one shade of green everywhere.** On Abandoned Carts it used to be grey until you pointed at it, which made a button that opens a conversation look like decoration. The icon on the orders list now uses the same green, so the same button looks the same on both screens.
* New: **An envelope beside each address on Abandoned Carts.** Clicking it opens a new message to that customer in whatever mail program you already use. Nothing is sent from the page. Like the WhatsApp button next to it, this one comes with BrikMentor: without it the envelope shows a padlock, and the address itself stays on screen and in your exports either way.
* Fix: **Connecting an ad account no longer gets undone by a background job.** Google Ads and Meta Ads credentials share one encrypted record, and a background job that had read that record minutes earlier wrote its old copy back when it finished. So a connection you had just made could vanish, and one you had just disconnected could come back, with nothing on screen explaining either. The record is now re-read immediately before every change, so neither can happen.
* Fix: **A history import that stops now says why, instead of freezing.** When the connection was lost part way through loading your three years of spend, the progress bar sat at "2 of 13" for good with no message anywhere. The card now says the import stopped and why, and tells you to connect again to resume it. The same applies when the import stops because you picked a different ad account.
* Fix: **Disconnecting a platform stops its history import.** Up to thirteen background jobs were left queued. Each one woke up, found no connection, and wrote the same line into the log, pushing out the errors that actually mattered. They are now cancelled with the disconnection, and the other platform's import is left running.
* Fix: **The log no longer presents routine notes as errors.** "Recent errors" is now "Recent activity", and each line is marked as either an error or a note, so a background job that correctly had nothing to do stops reading like a fault.
* Fix: **After connecting, the screen now tells you to choose an ad account.** The badge turned green as soon as the connection was made, but nothing is imported until you pick the account to pull spend from, and nothing said so. A short "One more step" note now appears until you have chosen one.
* Fix: **Manager accounts are called out in the Google Ads account list.** Google only offers the accounts your sign-in is attached to, so an agency often sees nothing but its manager account. Picking that one leaves you connected with an import that comes back empty. The list now says so and points you at the ad account ID you actually advertise from, with the manager ID going in the Manager (MCC) field.
* Fix: **Saving a manager (MCC) account ID no longer reports success when it failed.** It said "Saved." even when nothing had been written.
